...
namespace Impinj_Reader.Services
{
    public class ReaderService
    {
                ...
                else if (epc.Length == 12) // Pulsera
                {
                    ...
                    foreach (var tarima in _activeTarimas.Keys.ToList())
                    {
                        ...
                        foreach (var tarimaEpc in _activeTarimas[tarima])
                        {
                            ...
                            await _hubContext.Clients.Group("EPCs").SendAsync("sendMessage", new
                            {
                                Type = "Asociación",
                                Tarima = tarimaEpc,
                                Operador = epc,
                            ...
                            Console.WriteLine($"Mensaje enviado al grupo EPCs: {{ Type: Asociación, Tarima: {tarimaEpc}, Operador: {epc}, Timestamp: {DateTime.UtcNow} }}");
                        }
        ...
        private async void OnKeepaliveReceived(ImpinjReader reader)
        {
            Console.WriteLine($"Keepalive received from reader at {reader.Address}");

            // Enviar mensaje por SignalR al frontend
            await _hubContext.Clients.Group("Keepalives").SendAsync("sendMessage", new
            {
                Type = "Keepalive",
                ReaderIP = reader.Address,
                Timestamp = DateTime.UtcNow
            });
        }
        ...
        private async void OnConnectionLost(ImpinjReader reader)
        {
            ...
            // Notificar al frontend sobre la pérdida de conexión
            // Notificar al grupo "Keepalives" sobre la pérdida de conexión
            await _hubContext.Clients.Group("Keepalives").SendAsync("sendMessage", new
            {
                Type = "ConnectionLost",
                ReaderIP = reader.Address,
